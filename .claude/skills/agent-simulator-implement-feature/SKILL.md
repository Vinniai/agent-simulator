...
description: |
  Guide for implementing features in agent-simulator — a Swift CLI + WebSocket server
  ...
  (3) User asks "add feature X to agent-simulator", "implement <gesture>", "wire <new
...
# Implement a feature in agent-simulator

agent-simulator is a **CLI + WebSocket server**, not a SwiftUI app. There is no
...
│     update `skills/agent-simulator/` references when CLI / wire     │
│     surface changed                                           │
└──────────────────────────────────────────────────────────────┘
```
...
1. **Wire shape** — the JSON envelope on `agent-simulator serve` WS / `agent-simulator input` stdin. Field names, optional vs required, default values.
...
4. **Browser** — when the feature is user-facing:
   - Add the wire field handling to `Resources/Web/sim-input-bridge.js`
     (translate plugin dialect → agent-simulator wire).
...
- `skills/agent-simulator/SKILL.md` — the "What's wired vs what isn't" list.
- `skills/agent-simulator/references/cli.md` — new flags / subcommands.
- `skills/agent-simulator/references/wire-protocol.md` — new envelope shapes.
...
- [ ] `skills/agent-simulator/SKILL.md` "What's wired" list updated (if CLI/wire changed)
- [ ] `skills/agent-simulator/references/cli.md` updated (if CLI changed)
- [ ] `skills/agent-simulator/references/wire-protocol.md` updated (if wire changed)
