...
//
// Public API:
//   AgentSimReviewClient.install({ getUdid, getInspector })
//     — mount on /simulators/:udid. getInspector returns the
//       AXInspector instance created by sim-stream.js.
//
//   AgentSimReviewClient.attachToFocus({ host, getUdid, getInspector })
...
(function () {
  ...
  const SESSION_KEY = 'agentsim.review.session';
  const LEGACY_SESSION_KEY = 'baguette.review.session';
  const SESSION_CHANGE_EVENT = 'agentsim:session-change';

  // One-time copy from the legacy "baguette.*" key the first time a
  // post-rename build loads. Keeps the operator's active session
  // pointer + sourceRoot pref intact across the rename. The legacy
  // entries are left in place so an older build on the same machine
  // still works during a phased rollout.
  (function migrateLegacyKeys() {
    try {
      const pairs = [
        ['baguette.review.session',    'agentsim.review.session'],
        ['baguette.review.sourceRoot', 'agentsim.review.sourceRoot'],
        ['baguette.simTheme',          'agentsim.simTheme'],
        ['baguette.actionableBezel',   'agentsim.actionableBezel'],
      ];
    ...
    } catch (_) { /* private mode, etc. — proceed without migration */ }
  })();
  ...
  function installPage(opts) {
    ...
    activityHost.id = 'agentsim-activity-dock';
    ...
    selectionHost.id = 'agentsim-selection-dock';
  ...
  function attachToFocus(opts) {
    ...
    const recorder = recorderHost && window.AgentSimFlowRecorder && window.AgentSimFlowRecorder.mount({
      host: recorderHost,
      getSessionId,
      getUdid,
    });
    ...
    if (recordBtn && window.AgentSimFlowRecorder) {
      ...
      recordBtn.addEventListener('click', async () => {
        ...
        if (!isRecording) {
          window.AgentSimFlowRecorder.start({ sessionId: sid });
          isRecording = true;
          ...
          if (name === null) {
            ...
            await window.AgentSimFlowRecorder.stop({});
            return;
          }
          await window.AgentSimFlowRecorder.stop({ name });
