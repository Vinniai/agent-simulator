...
(function () {
  ...
  function ensurePatched() {
    ...
    window.SimInputBridge.makeTransport = function (session, log) {
      ...
      return function (payload) {
        if (state.active) {
          try {
            const wire = window.SimInputBridge.toAgentSimWire(payload, log);
            if (wire && wire.type) {
  ...
  function mount({ host, getSessionId, getUdid }) {
    ...
    window.addEventListener('agentsim:session-change', refresh);
    refresh();

    return {
      unmount() {
        window.removeEventListener('agentsim:session-change', refresh);
        host.innerHTML = '';
      },
      refresh,
    };
