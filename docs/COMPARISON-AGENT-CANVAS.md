...
- **baguette** — *live simulator + queued task runner* for any iOS-26
...
- Both can produce *screenshot + AX element JSON + operator comment*
  for a given screen. baguette via the review queue; agent-canvas via
...
- Both can call `argent` to drive the simulator (agent-canvas does
  this exclusively; baguette has it as a fallback for AX-only).
...
- **iOS-26 input dispatch.** The real reason baguette exists — the
...
## Positioning for baguette-as-a-product
...
agent reads manifest+comments, fixes      # 3. external agent driver
                                          #    via argent or baguette to drive
baguette serves live verification         # 4. live re-capture + visual diff
baguette review-tasks verify              # 5. pass/fail back into the queue
```

If "our project" stays on top of baguette, the agent-canvas pieces
worth borrowing:

1. **Route walker.** One-pass capture of every Expo Router route is
   genuinely useful and we don't have it. Plug into
   `baguette review-tasks bulk-create --from-routes <manifest.json>`.
...
- **Expo Router dependency.** baguette is app-agnostic and should
  stay so.
...
agent-canvas and baguette are **complements, not competitors**. The
clearest split: agent-canvas catalogues *what exists* across an Expo
Router app's surface; baguette captures *what's happening right now*
...
- *Liveness* from baguette (whatever moment the operator wants fixed)
- *Audit* from baguette (the code-change → before/after-snapshot trail
  per task)

If we were ever to merge the two, baguette is the wider foundation —
