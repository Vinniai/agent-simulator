...
- `agent-simulator key --code <KeyA…> [--modifiers shift,command] [--duration <s>]` — single keystroke.
- `agent-simulator type --text "<string>"` — typed as a sequence of keystrokes.
- Wire JSON `{ "type": "key", "code": "KeyA", "modifiers": ["shift"] }` and
  `{ "type": "type", "text": "hello" }` on `agent-simulator serve`'s WebSocket and
  `agent-simulator input`'s stdin.
...
- **No key repeat from CLI.** `agent-simulator key` emits one keystroke; for
