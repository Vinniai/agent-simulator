"""Convert an agent-canvas route manifest into a baguette bulk-create
envelope.
...
suitable for `baguette review-tasks bulk-create --file -`, so the
...
Usage:
    python3 agent_canvas_to_baguette.py \\
        ...
        | baguette review-tasks bulk-create --session-id review-xyz --file -
...
What this script does NOT do:
    - Import agent-canvas's PNG screenshots into baguette's snapshot
      store. Until that adapter lands, the bulk-created tasks reference
      `snapshotId` values that don't exist in baguette's review session.
...
def main() -> None:
    ...
    p.add_argument("--session-id", required=True,
                   help="Existing baguette review session id every task attaches to")
